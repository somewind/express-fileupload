    ...
    mv: (filePath, callback) => {
      return new Promise((resolve, reject) => {
        const handler = () => {
          ...
          return moveFunc(callbackWapper);
        };

        if (options.writePromise && options.writePromise instanceof Promise) {
          options.writePromise.then(handler);
        } else {
          handler();
        }
      });
